 ...
 * January 28, 2020
 *
 * Modified the string_set so that it includes functions for reversal.
 * Fixed some bugs with behavior slower than O(logN) for traversing the list.
 *
  ...
  class string_set
    {
      ...
      node *head;          // The head of the list will sit in a sentinel node
                           // (without any data in it).  This sentinel node
                           // will have a maximum width next list.  The head
                           // sentinel node should be in heap.

      int size;            // The number of elements in the set

      bool ascending;      // Determines if the string_set is sorted in ascending
                           // or descending order
    ...
    public:
      ...
      void reverse();                                    // Takes a string_set and puts it in reverse order
    ...
    private:
      ...
      const int get_height_of_next();
      
      // Sets up prev vector and moves to the desired location of the list 
      // (for adding, removal, etc.)
      void traverse(std::vector<node*> & prev, const std::string & target) const;

      void adjust_prev(std::vector<node*> & prev, node* & new_level, const int i) const;
  };
}
